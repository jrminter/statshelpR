...
var_interval = function(data, sigma0, conf_level = 0.95) {
  ...
  vNames <- c("Standard deviation",
              "Test statistic",
              "Degrees of freedom",
              paste("2-tailed test critical values, alpha=", sAlpha, "Lower T"),
              paste("2-tailed test critical values, alpha=", sAlpha, "Upper T"),
              paste0(sCI, "% Lower Confidence Interval for Standard Deviation"),
              paste0(sCI, "% Upper Confidence Interval for Standard Deviation"))
  vResult <- c(round(sqrt(v),4),
               round(testchi,4),
               round(df,0),
               round(qchisq(alpha/2,df),4),
               round(qchisq(1-alpha/2,df),4),
