...
![Optical diffraction patter from a TEM negative. The cross-hatch in te center is an artifact of printing in a book. We are interested in the spacing between the maxima and minima in the circular rings.](./inc/optical-diffraction.png)
...
Cs <- 2.0  # The microscope had a spherical aberraction coefficent of 2.0 mm

data(olk_data)
pander(olk_data)
```

Now lets's process the data. We will write a function to compute the
wavelength of the elctron beam.

```{r wavelengthFunction}
fWavelength <- function (fVoltk){
	# A function to compute the electron wavelendth from the voltake
...
Now we process the data in our datafame to make a linear equation
